...
*/
var pike = {animation:{}};
pike.animation.Animator = function(a) {
  this.duration_ = a;
  ...
  this.reverseLoop_ = this.running_ = this.started_ = !1
};
...
pike.animation.Animator.prototype.start = function() {
  this.running_ = this.started_ = !0
};
pike.animation.Animator.prototype.isRunning = function() {
  return this.running_
};
pike.animation.Animator.prototype.stop = function() {
  this.timeSinceLoopStart_ = this.loopsDone_ = 0;
  this.started_ = this.running_ = !1
};
pike.animation.Animator.prototype.pause = function() {
  this.running_ = !1
};
pike.animation.Animator.prototype.update = function(a) {
  if(this.started_) {
    this.running_ || (a = 0);
    this.timeSinceLoopStart_ += a;
    if(this.timeSinceLoopStart_ >= this.duration_ && (a = Math.floor(this.timeSinceLoopStart_ / this.duration_), this.timeSinceLoopStart_ %= this.duration_, this.repeatCount_ != pike.animation.Animator.INFINITE && a > this.repeatCount_ - this.loopsDone_ && (a = this.repeatCount_ - this.loopsDone_), this.loopsDone_ += a, this.reverseLoop_ = this.repeatBehavior_ == pike.animation.Animator.RepeatBehavior.REVERSE && 1 == this.loopsDone_ % 2, this.repeatCount_ != pike.animation.Animator.INFINITE && this.loopsDone_ == 
    this.repeatCount_)) {
      this.stop();
      return
    }
    a = this.timeSinceLoopStart_ / this.duration_;
    this.reverseLoop_ && (a = 0.99999999999999 - a);
    return a
  }
};
...
pike.animation.Animator.prototype.setDuration = function(a) {
...
pike.animation.Animator.prototype.setRepeatCount = function(a) {
  this.throwIfStarted_();
  if(1 > a && a != pike.animation.Animator.INFINITE) {
...
pike.animation.Animator.prototype.setRepeatBehavior = function(a) {
  return this.repeatBehavior_ = a
};
...
goog.string.compareVersions = function(a, b) {
  for(var c = 0, d = goog.string.trim(String(a)).split("."), e = goog.string.trim(String(b)).split("."), f = Math.max(d.length, e.length), g = 0;0 == c && g < f;g++) {
    var h = d[g] || "", l = e[g] || "", k = RegExp("(\\d*)(\\D*)", "g"), n = RegExp("(\\d*)(\\D*)", "g");
    do {
      var m = k.exec(h) || ["", "", ""], p = n.exec(l) || ["", "", ""];
...
goog.events.listen_ = function(a, b, c, d, e, f) {
  ...
  var g = g[e], h = goog.getUid(a), l;
  g.remaining_++;
  if(g[h]) {
    l = g[h];
    for(var k = 0;k < l.length;k++) {
      if(g = l[k], g.listener == c && g.handler == f) {
        if(g.removed) {
          break
        }
        d || (l[k].callOnce = !1);
  ...
  g = new goog.events.Listener(c, k, a, b, e, f);
  g.callOnce = d;
  k.src = a;
  k.listener = g;
  l.push(g);
  ...
  a.addEventListener ? a.addEventListener(b, k, e) : a.attachEvent(goog.events.getOnString_(b), k);
  ...
  if(a[b]) {
    ...
    try {
      for(var l = h.length, k = 0;k < l;k++) {
        var n = h[k];
        n && !n.removed && (f &= !1 !== goog.events.fireListener(n, e))
      }
    }finally {
  ...
  if(!goog.events.BrowserFeature.HAS_W3C_EVENT_SUPPORT) {
    ...
    try {
      if(g) {
        ...
        for(var m = k.length - 1;!l.propagationStopped_ && 0 <= m && f.remaining_;m--) {
          l.currentTarget = k[m], e &= goog.events.fireListeners_(f, k[m], c, !0, l)
        }
        if(h) {
          for(f = d[!1], f.remaining_ = f.count_, m = 0;!l.propagationStopped_ && m < k.length && f.remaining_;m++) {
            l.currentTarget = k[m], e &= goog.events.fireListeners_(f, k[m], c, !1, l)
          }
        }
      }else {
        e = goog.events.fireListener(a, l)
      }
    }finally {
      k && (k.length = 0)
    }
...
*/
pike.events = {};
pike.events.Update = function(a, b) {
  goog.events.Event.call(this, pike.events.Update.EVENT_TYPE, b);
  this.now = a
};
goog.inherits(pike.events.Update, goog.events.Event);
pike.events.Update.EVENT_TYPE = "update";
pike.events.Render = function(a, b) {
  goog.events.Event.call(this, pike.events.Render.EVENT_TYPE, b);
  this.now = a
};
goog.inherits(pike.events.Render, goog.events.Event);
pike.events.Render.EVENT_TYPE = "render";
pike.events.NewEntity = function(a, b) {
  goog.events.Event.call(this, pike.events.NewEntity.EVENT_TYPE, b);
  this.entity = a
};
goog.inherits(pike.events.NewEntity, goog.events.Event);
pike.events.NewEntity.EVENT_TYPE = "newentity";
pike.events.RemoveEntity = function(a, b) {
  goog.events.Event.call(this, pike.events.RemoveEntity.EVENT_TYPE, b);
  this.entity = a
};
goog.inherits(pike.events.RemoveEntity, goog.events.Event);
pike.events.RemoveEntity.EVENT_TYPE = "removeentity";
pike.events.Down = function(a, b, c, d) {
  goog.events.Event.call(this, pike.events.Down.EVENT_TYPE, d);
  this.posX = a;
  this.posY = b;
  this.domEvent = c
};
...
pike.events.Up = function(a, b, c, d, e) {
  goog.events.Event.call(this, pike.events.Up.EVENT_TYPE, e);
  this.posX = a;
  this.posY = b;
  this.isMoving = c;
  this.domEvent = d
...
pike.events.Move = function(a, b, c, d, e, f) {
  goog.events.Event.call(this, pike.events.Move.EVENT_TYPE, f);
  this.posX = a;
  this.posY = b;
  this.deltaX = c;
  this.deltaY = d;
...
pike.events.ChangePosition = function(a, b, c, d, e) {
  goog.events.Event.call(this, pike.events.ChangePosition.EVENT_TYPE, e);
  this.x = a;
  this.y = b;
  this.oldX = c;
  this.oldY = d
...
pike.events.ChangeSize = function(a, b, c, d, e) {
  goog.events.Event.call(this, pike.events.ChangeSize.EVENT_TYPE, e);
  this.w = a;
  this.h = b;
  this.oldW = c;
  this.oldH = d
...
pike.events.Collision = function(a, b, c, d, e, f) {
  goog.events.Event.call(this, pike.events.Collision.EVENT_TYPE, f);
  this.x = a;
  this.y = b;
  this.oldX = c;
  this.oldY = d;
...
*/
pike.graphics = {};
pike.graphics.Rectangle = function(a, b, c, d) {
  this.x = a || 0;
  this.y = b || 0;
  this.w = c || 0;
  this.h = d || 0
...
pike.graphics.Rectangle.prototype.equals = function(a) {
...
pike.graphics.Rectangle.prototype.intersects = function(a) {
  return this.x <= a.x + a.w && this.x + this.w >= a.x && this.y <= a.y + a.h && this.y + this.h >= a.y
};
pike.graphics.Rectangle.prototype.intersection = function(a) {
  var b = Math.max(this.x, a.x), c = Math.max(this.y, a.y), d = Math.min(this.x + this.w, a.x + a.w) - b;
  a = Math.min(this.y + this.h, a.y + a.h) - c;
  return 0 >= d || 0 >= a ? null : new pike.graphics.Rectangle(b, c, d, a)
};
pike.graphics.Rectangle.prototype.covers = function(a) {
  return a.x >= this.x && a.y >= this.y && a.x + a.w <= this.x + this.w && a.y + a.h <= this.y + this.h
};
pike.graphics.Rectangle.prototype.containsPoint = function(a, b) {
  return a >= this.x && a < this.x + this.w && b >= this.y && b < this.y + this.h
};
pike.graphics.Rectangle.prototype.convexHull = function(a) {
  var b = Math.min(this.x, a.x), c = Math.min(this.y, a.y), d = Math.max(this.x + this.w, a.x + a.w) - b;
  a = Math.max(this.y + this.h, a.y + a.h) - c;
  return new pike.graphics.Rectangle(b, c, d, a)
};
pike.graphics.Rectangle.prototype.getOverlappingGridCells = function(a, b, c, d) {
  var e = Math.max(0, Math.floor(this.x / a)), f = Math.max(0, Math.floor(this.y / b));
  a = Math.min(c - e, Math.floor((this.x + this.w) / a) - e + 1);
  b = Math.min(d - f, Math.floor((this.y + this.h) / b) - f + 1);
  return new pike.graphics.Rectangle(e, f, a, b)
};
pike.graphics.Cluster = function(a, b, c) {
  this.clusterSize_ = a;
  this.bounds_ = new pike.graphics.Rectangle(0, 0, b, c);
  this.clusters_ = [];
  this.idToClusterBounds_ = {}
};
pike.graphics.Cluster.prototype.setSize = function(a, b) {
  this.bounds_.w = a;
  this.bounds_.h = b
};
...
pike.graphics.Cluster.prototype.getIdToClusterBounds = function(a) {
  return this.idToClusterBounds_[a]
};
pike.graphics.Cluster.prototype.setIdToClusterBounds = function(a, b) {
  this.idToClusterBounds_[a] = b
};
pike.graphics.Cluster.prototype.build = function() {
  this.clusters_ = [];
  for(var a = 0;a < Math.ceil(this.bounds_.h / this.clusterSize_);a++) {
    this.clusters_[a] = [];
    for(var b = 0;b < Math.ceil(this.bounds_.w / this.clusterSize_);b++) {
      this.clusters_[a][b] = []
    }
  }
};
pike.graphics.Cluster.prototype.addToClusters = function(a, b) {
  b = b || a.getBounds().getOverlappingGridCells(this.clusterSize_, this.clusterSize_, this.clusters_[0].length, this.clusters_.length);
  for(var c = b.y;c < b.y + b.h;c++) {
    for(var d = b.x;d < b.x + b.w;d++) {
      this.clusters_[c][d].push(a)
    }
  }
  this.idToClusterBounds_[a.id] = b;
  goog.DEBUG && window.console.log("[pike.graphics.Cluster] entity " + a.id + " is added to cluster " + b);
  return b
};
pike.graphics.Cluster.prototype.removeFromClusters = function(a, b) {
  b = b || this.idToClusterBounds_[a.id];
  for(var c = b.y;c < b.y + b.h;c++) {
    for(var d = b.x;d < b.x + b.w;d++) {
      goog.array.remove(this.clusters_[c][d], a)
    }
  }
  goog.DEBUG && window.console.log("[pike.graphics.Cluster] entity " + a.id + " is removed from cluster " + b)
};
// Input 22
/*
 Dual licensed under the MIT or GPL licenses.
*/
pike.core = {};
pike.core.Entity = function(a) {
  goog.events.EventTarget.call(this);
  this.id = goog.getUid(this);
  this.handler = new goog.events.EventHandler(this);
  for(var b = 0;b < arguments.length;b++) {
    if("function" !== typeof arguments[b]) {
      throw"Argument is not a function " + arguments[b];
    }
    arguments[b].call(this)
  }
};
...
pike.core.Viewport.prototype.setSize = function(a, b) {
  var c = this.viewport_.w, d = this.viewport_.h;
  this.viewport_.w = a;
  this.viewport_.h = b;
  goog.DEBUG && window.console.log("[pike.core.Viewport] changesize");
  this.dispatchEvent(new pike.events.ChangeSize(this.viewport_.w, this.viewport_.h, c, d, this))
};
pike.core.Viewport.prototype.setPosition = function(a, b) {
  var c = this.viewport_.x, d = this.viewport_.y;
  this.viewport_.x = a;
  this.viewport_.y = b;
  goog.DEBUG && window.console.log("[pike.core.Viewport] changeposition");
  this.dispatchEvent(new pike.events.ChangePosition(this.viewport_.x, this.viewport_.y, c, d, this))
};
...
pike.core.GameWorld.prototype.setSize = function(a, b) {
  var c = this.bounds_.w, d = this.bounds_.h;
  this.bounds_.w = a;
  this.bounds_.h = b;
  goog.DEBUG && window.console.log("[pike.core.GameWorld] changesize");
  this.dispatchEvent(new pike.events.ChangeSize(this.bounds_.w, this.bounds_.h, c, d, this))
};
...
pike.core.Timer.prototype.stop = function() {
  this.requestID_ && window.cancelAnimationFrame(this.requestID_)
};
...
pike.core.Stage.prototype.getLayer = function(a) {
  for(var b = 0;b < this.layers_.length;b++) {
    if(this.layers_[b].name == a) {
      return this.layers_[b]
    }
  }
  throw Error("Layer with name " + a + " does not exist.");
};
pike.core.Stage.prototype.addLayer = function(a) {
  this.setLayerSize_(a);
  this.layers_.push(a);
  this.getRootElement().appendChild(a.getScreen().canvas)
};
pike.core.Stage.prototype.getRootElement = function() {
  return this.rootElement_
};
pike.core.Stage.prototype.onRender = function(a) {
  for(a = 0;a < this.layers_.length;a++) {
    this.layers_[a].onRender()
  }
};
pike.core.Stage.prototype.onViewportChangeSize = function(a) {
  this.setViewportSize_(a.w, a.h)
};
pike.core.Stage.prototype.onViewportChangePosition = function(a) {
  this.setViewportPosition_(a.x, a.y)
};
pike.core.Stage.prototype.onGameWorldChangeSize = function(a) {
  this.setGameWorldSize_(a.w, a.h)
};
pike.core.Stage.prototype.createRootElement_ = function() {
  (this.rootElement_ = document.getElementById(pike.core.Stage.ELEMENT_ID)) ? this.rootElement_.innerHTML = "" : (this.rootElement_ = document.createElement("div"), this.rootElement_.setAttribute("id", pike.core.Stage.ELEMENT_ID), document.getElementsByTagName("body")[0].appendChild(this.rootElement_));
  this.rootElement_.style.position = "relative"
};
pike.core.Stage.prototype.setLayerSize_ = function(a) {
  a.setViewportSize(this.viewport_.w, this.viewport_.h);
  a.setViewportPosition(this.viewport_.x, this.viewport_.y);
  a.setGameWorldSize(this.gameWorld_.w, this.gameWorld_.h)
};
pike.core.Stage.prototype.setViewportSize_ = function(a, b) {
  if(this.viewport_.w != a || this.viewport_.h != b) {
    this.viewport_.w = a;
    this.viewport_.h = b;
    this.rootElement_.style.width = this.viewport_.w + "px";
    this.rootElement_.style.height = this.viewport_.h + "px";
    (this.gameWorld_.w < a || this.gameWorld_.h < b) && this.setGameWorldSize_(a, b);
    for(var c = 0;c < this.layers_.length;c++) {
      this.setLayerSize_(this.layers_[c])
    }
  }
};
pike.core.Stage.prototype.setGameWorldSize_ = function(a, b) {
  this.gameWorld_.w = Math.max(a, this.viewport_.w);
  this.gameWorld_.h = Math.max(b, this.viewport_.h);
  for(var c = 0;c < this.layers_.length;c++) {
    this.setLayerSize_(this.layers_[c])
  }
};
pike.core.Stage.prototype.setViewportPosition_ = function(a, b) {
  this.viewport_.x = a;
  this.viewport_.y = b;
  for(var c = 0;c < this.layers_.length;c++) {
    this.layers_[c].setViewportPosition(this.viewport_.x, this.viewport_.y)
  }
};
// Input 23
/*
...
pike.input.InputHandlerBase = function() {
  ...
  this.stopDomEvents_ = !0
};
goog.inherits(pike.input.InputHandlerBase, goog.events.EventTarget);
pike.input.InputHandlerBase.prototype.onDownDomEvent = function(a) {
  var b = this.lastMoveCoordinates_ = this.getInputCoordinates_(a);
  goog.DEBUG && window.console.log("[pike.core.InputHanderBase] down " + b.posX + ", " + b.posY);
  this.dispatchEvent(new pike.events.Down(b.posX, b.posY, a, this));
  this.stopEventIfRequired_(a)
};
pike.input.InputHandlerBase.prototype.onUpDomEvent = function(a) {
  var b = this.getInputCoordinates_(a);
  goog.DEBUG && window.console.log("[pike.core.InputHanderBase] up " + b.posX + ", " + b.posY + ", moving: " + this.moving_);
  this.dispatchEvent(new pike.events.Up(b.posX, b.posY, this.moving_, a, this));
  this.stopEventIfRequired_(a);
  this.moving_ = !1
};
pike.input.InputHandlerBase.prototype.onMoveDomEvent = function(a) {
  var b = this.getInputCoordinates_(a), c = b.posX - this.lastMoveCoordinates_.posX, d = b.posY - this.lastMoveCoordinates_.posY;
  !this.moving_ && Math.sqrt(c * c + d * d) > this.moveThreshold_ && (this.moving_ = !0);
  this.moving_ && (goog.DEBUG && window.console.log("[pike.core.InputHanderBase] move " + b.posX + ", " + b.posY + ", deltaX: " + c + ", deltaY: " + d), this.dispatchEvent(new pike.events.Move(b.posX, b.posY, c, d, a, this)), this.lastMoveCoordinates_ = b);
  this.stopEventIfRequired_(a)
};
pike.input.InputHandlerBase.prototype.stopEventIfRequired_ = function(a) {
  this.stopDomEvents_ && (a.stopPropagation(), a.preventDefault())
};
pike.input.InputHandlerBase.prototype.getInputCoordinates_ = function(a) {
  return{posX:a.offsetX + this.viewport_.x, posY:a.offsetY + this.viewport_.y}
};
pike.input.InputHandlerBase.prototype.onViewportChangePosition = function(a) {
  this.viewport_.x = a.x;
  this.viewport_.y = a.y
};
...
pike.input.MouseInputHandler.prototype.setEventTarget = function(a) {
  this.element_ = a;
  this.attachDomListeners_()
};
pike.input.MouseInputHandler.prototype.attachDomListeners_ = function() {
  var a = this.element_;
  this.handler.listen(a, goog.events.EventType.MOUSEDOWN, goog.bind(this.onDownDomEvent, this));
  this.handler.listen(a, goog.events.EventType.MOUSEUP, goog.bind(this.onUpDomEvent, this));
  this.handler.listen(a, goog.events.EventType.MOUSEMOVE, goog.bind(this.onMoveDomEvent, this));
  this.handler.listen(a, goog.events.EventType.MOUSEOUT, goog.bind(this.onMouseOut, this))
};
pike.input.MouseInputHandler.prototype.onDownDomEvent = function(a) {
  this.mouseDown_ = !0;
  pike.input.InputHandlerBase.prototype.onDownDomEvent.call(this, a)
};
pike.input.MouseInputHandler.prototype.onUpDomEvent = function(a) {
  this.mouseDown_ = !1;
  pike.input.InputHandlerBase.prototype.onUpDomEvent.call(this, a)
};
pike.input.MouseInputHandler.prototype.onMoveDomEvent = function(a) {
  this.mouseDown_ && pike.input.InputHandlerBase.prototype.onMoveDomEvent.call(this, a)
};
...
pike.input.TouchInputHandler.prototype.setEventTarget = function(a) {
  this.element_ = a;
  this.attachDomListeners_()
};
pike.input.TouchInputHandler.prototype.attachDomListeners_ = function() {
  var a = this.element_;
  this.handler.listen(a, goog.events.EventType.TOUCHSTART, goog.bind(this.onDownDomEvent, this));
  this.handler.listen(a, goog.events.EventType.TOUCHEND, goog.bind(this.onUpDomEvent, this));
  this.handler.listen(a, goog.events.EventType.TOUCHMOVE, goog.bind(this.onMoveDomEvent, this))
};
pike.input.TouchInputHandler.prototype.onDownDomEvent = function(a) {
  this.lastInteractionCoordinates_ = this.getInputCoordinates_(a);
  pike.input.InputHandlerBase.prototype.onDownDomEvent.call(this, a)
};
pike.input.TouchInputHandler.prototype.onUpDomEvent = function(a) {
  goog.DEBUG && window.console.log("[pike.core.TouchHanderBase] up " + this.lastInteractionCoordinates_.posX + ", " + this.lastInteractionCoordinates_.posY + ", moving: " + this.moving_);
  this.dispatchEvent(new pike.events.Up(this.lastInteractionCoordinates_.posX, this.lastInteractionCoordinates_.posY, this.moving_, a, this));
  this.stopEventIfRequired_(a);
  this.lastInteractionCoordinates_ = null;
  this.moving_ = !1
};
pike.input.TouchInputHandler.prototype.onMoveDomEvent = function(a) {
  this.lastInteractionCoordinates_ = this.getInputCoordinates_(a);
  pike.input.InputHandlerBase.prototype.onMoveDomEvent.call(this, a)
};
// Input 24
...
pike.layers.Layer.prototype.renderOffScreen_ = function() {
  ...
  this.dispatchEvent(new pike.events.Render((new Date).getTime(), this));
  this.offScreen_.isDirty = !1;
  this.screen_.isDirty = !0;
  goog.DEBUG && window.console.log("[pike.core.Layer] " + this.name + " redraw offScreen")
};
pike.layers.Layer.prototype.renderScreen_ = function() {
  ...
  this.screen_.isDirty = !1;
  goog.DEBUG && window.console.log("[pike.core.Layer] " + this.name + " redraw screen")
};
pike.layers.Layer.prototype.setViewportSize = function(a, b) {
  this.viewport_.w = a;
  this.viewport_.h = b;
  this.screen_.canvas.width = this.viewport_.w;
  this.screen_.canvas.height = this.viewport_.h;
  this.screen_.isDirty = !0;
  this.hasDirtyManager() && this.dirtyManager.setSize(this.viewport_.w, this.viewport_.h)
};
pike.layers.Layer.prototype.setGameWorldSize = function(a, b) {
  this.gameWorld_.w = a;
  this.gameWorld_.h = b;
  this.offScreen_.canvas.width = this.gameWorld_.w;
  this.offScreen_.canvas.height = this.gameWorld_.h;
  this.offScreen_.isDirty = !0
};
pike.layers.Layer.prototype.setViewportPosition = function(a, b) {
  this.viewport_.x = a;
  this.viewport_.y = b;
  this.getScreen().isDirty = !0;
  this.hasDirtyManager() && this.dirtyManager.setPosition(this.viewport_.x, this.viewport_.y)
};
pike.layers.Layer.prototype.setDirtyManager = function(a) {
  this.dirtyManager = a
};
...
pike.layers.Layer.prototype.addEntity = function(a) {
  this.entities_.push(a);
  a.layer = this;
  this.dispatchEvent(new pike.events.NewEntity(a, this));
  goog.DEBUG && window.console.log("[pike.core.Layer] Layer: " + this.name + " has newentity #" + a.id)
};
pike.layers.Layer.prototype.removeEntity = function(a) {
  a.dispose();
  goog.array.remove(this.entities_, a);
  delete a.layer;
  this.dispatchEvent(new pike.events.RemoveEntity(a, this));
  goog.DEBUG && window.console.log("[pike.core.Layer] removeentity")
};
pike.layers.Layer.prototype.dispatchEvent = function(a) {
  for(var b = 0;b < this.entities_.length;b++) {
    this.entities_[b].dispatchEvent(a)
  }
  goog.events.EventTarget.prototype.dispatchEvent.call(this, a)
};
pike.layers.ClusterLayer = function(a, b) {
  pike.layers.Layer.call(this, a);
  this.clusters_ = new pike.graphics.Cluster(b, 0, 0);
  this.visibleClusterBounds_ = {};
  this.cache_ = [];
  this.cacheDirty_ = !0;
  this.cacheUnsorted_ = !1
};
goog.inherits(pike.layers.ClusterLayer, pike.layers.Layer);
pike.layers.ClusterLayer.prototype.getCluster = function() {
  return this.clusters_
};
pike.layers.ClusterLayer.prototype.dispatchEvent = function(a) {
  this.cacheDirty_ ? this.resetCache_() : this.cacheUnsorted_ && this.sortCache_();
  for(var b = 0;b < this.cache_.length;b++) {
    var c = this.cache_[b];
    c.getBounds().intersects(this.viewport_) && c.dispatchEvent(a)
  }
  goog.events.EventTarget.prototype.dispatchEvent.call(this, a)
};
pike.layers.ClusterLayer.prototype.addEntity = function(a) {
  pike.layers.Layer.prototype.addEntity.call(this, a);
  this.handler.listen(a, pike.events.ChangePosition.EVENT_TYPE, goog.bind(this.onEntityMove, this));
  0 != this.clusters_.getClusters().length && this.clusters_.addToClusters(a).intersects(this.visibleClusterBounds_) && (this.cache_.push(a), this.cacheUnsorted_ = !0)
};
pike.layers.ClusterLayer.prototype.removeEntity = function(a) {
  this.clusters_.removeFromClusters(a);
  pike.layers.Layer.prototype.removeEntity.call(this, a)
};
pike.layers.ClusterLayer.prototype.resetCache_ = function() {
  for(var a = this.cache_ = [], b = this.visibleClusterBounds_.y;b < this.visibleClusterBounds_.y + this.visibleClusterBounds_.h;b++) {
    for(var c = this.visibleClusterBounds_.x;c < this.visibleClusterBounds_.x + this.visibleClusterBounds_.w;c++) {
      for(var d = this.clusters_.getClusters()[b][c], e = 0;e < d.length;e++) {
        -1 == a.indexOf(d[e]) && a.push(d[e])
      }
    }
  }
  goog.DEBUG && window.console.log("[pike.layers.ClusterLayer] resetcache");
  this.sortCache_();
  this.cacheUnsorted_ = this.cacheDirty_ = !1
};
pike.layers.ClusterLayer.prototype.sortCache_ = function() {
  this.cache_.sort(function(a, b) {
    var c = a.getBounds(), d = b.getBounds();
    return c.y + c.h - (d.y + d.h)
  });
  this.cacheUnsorted_ = !1;
  goog.DEBUG && window.console.log("[pike.layers.ClusterLayer] sortcache")
};
pike.layers.ClusterLayer.prototype.resetClusters_ = function() {
  this.clusters_.build();
  goog.DEBUG && window.console.log("[pike.layers.ClusterLayer] resetcluster");
  for(var a = 0;a < this.entities_.length;a++) {
    this.clusters_.addToClusters(this.entities_[a])
  }
};
pike.layers.ClusterLayer.prototype.updateVisibleClusters_ = function() {
  var a = this.viewport_.getOverlappingGridCells(this.clusters_.getClusterSize(), this.clusters_.getClusterSize(), this.clusters_.getClusters()[0].length, this.clusters_.getClusters().length);
  a.equals(this.visibleClusterBounds_) || (this.visibleClusterBounds_ = a, this.cacheDirty_ = !0)
};
pike.layers.ClusterLayer.prototype.setViewportSize = function(a, b) {
  pike.layers.Layer.prototype.setViewportSize.call(this, a, b);
  this.setGameWorldSize.call(this, Math.max(this.gameWorld_.w, a), Math.max(this.gameWorld_.h, b));
  this.updateVisibleClusters_()
};
pike.layers.ClusterLayer.prototype.setViewportPosition = function(a, b) {
  pike.layers.Layer.prototype.setViewportPosition.call(this, a, b);
  this.updateVisibleClusters_()
};
pike.layers.ClusterLayer.prototype.setGameWorldSize = function(a, b) {
  pike.layers.Layer.prototype.setGameWorldSize.call(this, a, b);
  this.clusters_.setSize(a, b);
  this.resetClusters_()
};
pike.layers.ClusterLayer.prototype.onEntityMove = function(a) {
  var b = a.target, c = b.getBounds().getOverlappingGridCells(this.clusters_.getClusterSize(), this.clusters_.getClusterSize(), this.clusters_.getClusters()[0].length, this.clusters_.getClusters().length), d = this.clusters_.getIdToClusterBounds(b.id);
  d.equals(c) || this.moveObjectBetweenClusters_(b, d, c);
  c.intersects(this.visibleClusterBounds_) && a.y != a.oldY && (this.cacheUnsorted_ = !0)
};
pike.layers.ClusterLayer.prototype.moveObjectBetweenClusters_ = function(a, b, c) {
  this.clusters_.removeFromClusters(a, b);
  this.clusters_.addToClusters(a, c);
  this.clusters_.setIdToClusterBounds(a.id, c);
  c.intersects(this.visibleClusterBounds_) ? goog.array.contains(this.cache_, a) || this.cache_.push(a) : goog.array.remove(this.cache_, a)
};
pike.layers.ObstacleLayer = function(a) {
  pike.layers.Layer.call(this, a)
};
goog.inherits(pike.layers.ObstacleLayer, pike.layers.Layer);
pike.layers.ObstacleLayer.prototype.setViewportSize = function(a, b) {
  pike.layers.Layer.prototype.setViewportSize.call(this, a, b);
  this.screen_.isDirty = !1
};
pike.layers.ObstacleLayer.prototype.setViewportPosition = function(a, b) {
  pike.layers.Layer.prototype.setViewportPosition.call(this, a, b);
  this.screen_.isDirty = !1
};
pike.layers.ObstacleLayer.prototype.renderOffScreen_ = function() {
  ...
  this.screen_.isDirty = !1;
  goog.DEBUG && window.console.log("[pike.core.ObstacleLayer] " + this.name + " redraw offScreen")
};
pike.layers.ObstacleLayer.prototype.onEntityChangePosition = function(a) {
  var b = a.target;
  0 == this.offScreen_.context.getImageData(b.x, b.y, 1, 1).data[3] && 0 == this.offScreen_.context.getImageData(b.x + b.w, b.y, 1, 1).data[3] && 0 == this.offScreen_.context.getImageData(b.x + b.w, b.y + b.h, 1, 1).data[3] && 0 == this.offScreen_.context.getImageData(b.x, b.y + b.h, 1, 1).data[3] || a.target.dispatchEvent(new pike.events.Collision(a.x, a.y, a.oldX, a.oldY, "obstacle", a.target))
};
pike.layers.DirtyManager = function(a) {
  this.viewport_ = new pike.graphics.Rectangle(0, 0, 0, 0);
  this.handler = new goog.events.EventHandler(this);
  this.dirtyRect_ = null;
  this.allDirtyThreshold_ = void 0 == a ? 0.5 : a;
  this.allDirty_ = !0;
  this.markAllDirty()
};
...
pike.layers.DirtyManager.prototype.markDirty = function(a) {
  (a.w || a.h) && !this.allDirty_ && (a = this.viewport_.intersection(a)) && (this.dirtyRect_ = this.dirtyRect_ ? this.dirtyRect_.convexHull(a) : a, this.dirtyRect_.w * this.dirtyRect_.h > this.allDirtyThreshold_ * this.viewport_.w * this.viewport_.h && this.markAllDirty())
};
...
pike.layers.DirtyManager.prototype.setSize = function(a, b) {
  this.viewport_.w = a;
  this.viewport_.h = b;
  this.markAllDirty()
};
pike.layers.DirtyManager.prototype.setPosition = function(a, b) {
  this.viewport_.x = a;
  this.viewport_.y = b
};
// Input 26
...
pike.components.Collision = function() {
  this.setCollisionBounds = function(a, b, c, d) {
    this.collisionBounds_ = new pike.graphics.Rectangle(a, b, c, d);
    return this
  };
  this.checkCollision = function(a) {
    for(var b = this.layer.getCluster().getIdToClusterBounds(this.id), b = this.layer.getCluster().getClusters()[b.y][b.x], c = 0;c < b.length;c++) {
      this.id != b[c].id && this.getCollisionBounds_(this).intersects(this.getCollisionBounds_(b[c])) && this.dispatchEvent(new pike.events.Collision(a.x, a.y, a.oldX, a.oldY, b[c], this))
    }
  };
  this.getCollisionBounds_ = function(a) {
    return a.collisionBounds_ ? new pike.graphics.Rectangle(a.getBounds().x + a.collisionBounds_.x, a.getBounds().y + a.collisionBounds_.y, a.getBounds().w - a.collisionBounds_.w, a.getBounds().h - a.collisionBounds_.h) : a.getBounds()
  };
...
pike.components.Sprite = function() {
  ...
  this.h = this.w = 32;
  this.spriteSource = {};
  this.setSprite = function(a, b, c, d, e, f, g) {
    this.spriteSource.image = a;
    this.spriteSource.x = b;
    this.spriteSource.y = c;
    this.spriteSource.w = d;
    this.spriteSource.h = e;
    this.spriteSource.row = 0;
    this.spriteSource.numberOfFrames = f || 1;
    this.spriteSource.currentFrame = 0;
    this.spriteSource.animator = new pike.animation.Animator(g || 1E3);
    this.spriteSource.animator.start();
    this.spriteSource.lastUpdate = (new Date).getTime();
    return this
  };
  this.onSpriteUpdate = function(a) {
    a = a.now;
    var b = a - this.spriteSource.lastUpdate;
    this.spriteSource.lastUpdate = a;
    a = this.spriteSource.animator.update(b);
    this.spriteSource.currentFrame = Math.floor(a * this.spriteSource.numberOfFrames);
    this.setDirty(this.getBounds())
  };
  this.setSpriteRow = function(a, b) {
    Math.abs(a) > Math.abs(b) ? 0 <= a ? this.spriteSource.row = pike.components.Sprite.RIGHT : this.spriteSource.row = pike.components.Sprite.LEFT : 0 <= b ? this.spriteSource.row = pike.components.Sprite.DOWN : this.spriteSource.row = pike.components.Sprite.UP
  };
  this.onSpriteRender = function(a) {
  ...
  this.setDirty = function(a) {
    this.layer.hasDirtyManager() ? this.layer.dirtyManager.markDirty(a) : this.layer.getOffScreen().isDirty = !0
  }
};
...
pike.components.Image = function() {
  ...
  this.onImageRender = function(a) {
    this.layer.getOffScreen().context.drawImage(this.image, 0, 0, this.w, this.h, 0, 0, this.w, this.h)
  }
};
pike.components.Watch = function() {
  this.watchMe = function(a, b) {
    var c = a.getBounds(), d = b.getBounds(), e = c.x, f = c.y;
    this.x < this.leftInnerBoundary(c) && (c.x = Math.max(0, Math.min(Math.floor(this.x - 0.25 * c.w), d.w - c.w)));
    this.x + this.w > this.rightInnerBoundary(c) && (c.x = Math.max(0, Math.min(Math.floor(this.x + this.w - 0.75 * c.w), d.w - c.w)));
    this.y < this.topInnerBoundary(c) && (c.y = Math.max(0, Math.min(Math.floor(this.y - 0.25 * c.h), d.h - c.h)));
    this.y + this.h > this.bottomInnerBoundary(c) && (c.y = Math.max(0, Math.min(Math.floor(this.y + this.h - 0.75 * c.h), d.h - c.h)));
    e == c.x && f == c.y || a.setPosition(c.x, c.y)
  };
  this.rightInnerBoundary = function(a) {
    return a.x + 0.75 * a.w
  };
  this.leftInnerBoundary = function(a) {
    return a.x + 0.25 * a.w
  };
  this.topInnerBoundary = function(a) {
    return a.y + 0.25 * a.h
  };
  this.bottomInnerBoundary = function(a) {
    return a.y + 0.75 * a.h
  }
};
// Input 27
...
pike.assets.ImageManager.prototype.load = function(a, b, c) {
  var d = [], e;
  for(e in a) {
    d.push({key:e, path:a[e]})
  }
  if(0 == d.length) {
    c && c(0, 0, null, null, !0), b && b()
  }else {
    for(a = {loaded:0, total:d.length}, e = 0;e < d.length;e++) {
      this.loadImage_(d[e], a, b, c)
    }
  }
};
pike.assets.ImageManager.prototype.get = function(a) {
  return this.images_[a]
};
pike.assets.ImageManager.prototype.loadImage_ = function(a, b, c, d) {
  var e = this, f = new Image;
  f.onload = function() {
    e.images_[a.key] = f;
    e.onItemLoaded_(a, b, c, d, !0)
  };
  f.onerror = function() {
    e.onItemLoaded_(a, b, c, d, !1)
  };
  f.src = a.path
};
pike.assets.ImageManager.prototype.onItemLoaded_ = function(a, b, c, d, e) {
  b.loaded++;
  d && d(b.loaded, b.total, a.key, a.path, e);
  b.loaded == b.total && c && c()
};
